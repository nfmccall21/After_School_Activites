...
RSpec.describe RegistrationsController, type: :controller do
  ...
  describe 'POST #approve' do
    ...
    context 'when spots are full' do
      let!(:student) { Student.create!(firstname: 'Nicholas', lastname: 'McCall', grade: 5, homeroom: 'homeroom 1') }
      let!(:activity) { Activity.create!(title: "test", description: "test", spots: 1, chaperone: "test", time_start: DateTime.parse('3 am').to_time, time_end: DateTime.parse('4 am').to_time, day: "Tuesday") } # Only 1 spot available
      let!(:filled_registration) { Registration.create!(student_id: student.id, activity_id: activity.id, status: 'Enrolled') }
      let!(:pending_registration) { Registration.create!(student_id: student.id, activity_id: activity.id, status: 'Pending') }
      ...
      it 'does not approve the registration and shows a notice' do
        ...
        expect(pending_registration.status).to eq('Pending') # Should not change to 'Enrolled'
      end
    end
  end
